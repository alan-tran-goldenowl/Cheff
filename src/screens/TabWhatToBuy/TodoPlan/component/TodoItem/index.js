...
const ToDoItem = ({item, onPress, navigation}) => {
  ...
  const onDelete = () => {
    Alert.alert('Nhắc nhở', 'Bạn chắc chắn muốn xóa ? ', [
      {
        text: 'Bỏ',
        style: 'cancel',
      },
      {
        ...
        onPress: () =>
          dispatch(
            deletePlanToBuy({userId: userFirebase.uid, planId: item.id}),
          ),
      },
    ]);
  };
      ...
      <View
        ...
        <View style={{...styles.row, marginBottom: 5}}>
          <Text style={styles.status}>
            {getProgress().completed} trong {getProgress().all} hoàn thành
          </Text>
          <Text style={styles.percent}>{getProgress().percent}%</Text>
        </View>

        <ProgressBar key="test" completed={getProgress().percent} />
      </View>

      <RBSheet ref={bottomSheetRef} closeOnDragDown duration={250}>
