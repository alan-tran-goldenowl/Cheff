...
const ActivityItem = ({item, navigation}) => {
  ...
  const goToDetails = () => {
    switch (item.action) {
      ...
      case 2:
      case 3:
        return navigateAction('FoodDetail', {key: item.key, from: 'Activity'});
      case 4:
      case 5:
        return navigateAction('CreatePlan', {id: item.key});
      case 6:
        return 'deleted';
      default:
        return 'viewed';
    ...
    <TouchableOpacity style={styles.container} onPress={goToDetails}>
      ...
      <View style={styles.textContainer}>
        <Text style={styles.text}>
          You <Text style={{fontWeight: 'bold'}}>{renderAction()}</Text>{' '}
          {item.name} {(item.action === 4 || item.action === 5) && 'plan'}
        </Text>
